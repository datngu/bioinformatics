...
Nguyên tắc trung tâm của sinh học phân tử mô tả dòng chảy thông tin theo chiều: **ADN → ARN → Protein → Chức năng tế bào**. Mỗi bước chuyển hóa này là một lớp thông tin riêng biệt, và trong ung thư, mỗi lớp có thể bị phá vỡ theo cách riêng của nó. **Omics** là tập hợp các ngành khoa học đo lường toàn hệ thống từng lớp thông tin đó — không đo một hoặc vài phân tử được chọn trước mà đo **toàn bộ** ADN, ARN, protein, hay chất chuyển hóa của một mẫu sinh học. Kết hợp nhiều lớp omics lại gọi là **đa omics** (multi-omics).

| Câu hỏi cần trả lời | Lớp đo lường | Ngành |
|---------------------|-------------|-------|
| ADN ung thư thay đổi những gì? | Biến thể ADN | Genomics |
| Gen nào đang được bật và tắt? | Lượng ARN | Transcriptomics |
| Vùng ADN nào bị "khóa im lặng"? | Trạng thái epigenome | Epigenomics |
...
Khi giải trình tự bộ gen của một khối u bằng **whole-genome sequencing** (WGS: giải trình tự toàn bộ bộ gen) hay **whole-exome sequencing** (WES: giải trình tự chỉ các vùng mã hóa protein, tức exome, chiếm khoảng 1,5% bộ gen nhưng chứa phần lớn đột biến chức năng), ta thu được một bức tranh toàn diện về các biến đổi ADN soma. Phổ biến nhất là **biến thể nucleotide đơn** (single nucleotide variant, SNV: thay đổi một nucleotide), **indel** (insertion/deletion: thêm hoặc bớt một đến vài nucleotide), và **biến thể số bản sao** (copy number variation, CNV: một đoạn ADN lớn bị nhân lên nhiều lần hay bị xóa đi).
...
Ung thư tiến hóa dưới áp lực chọn lọc của thuốc. Dòng vô tính thiểu số mang đột biến kháng thuốc có thể phát triển thành quần thể chiếm ưu thế sau điều trị. Với ctDNA, ta có thể theo dõi **entropy di truyền khối u** theo thời gian thực qua xét nghiệm máu định kỳ — phát hiện đột biến kháng thuốc xuất hiện sớm hơn vài tháng so với tái phát lâm sàng. Điều này cho phép điều chỉnh phác đồ chủ động thay vì phản ứng sau khi ung thư đã tái phát hoàn toàn — hoàn toàn nằm ngoài tầm với của mọi kỹ thuật không dựa trên giải trình tự ADN.
